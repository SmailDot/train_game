...
## [2025-12-05] - 🎉 v5 Final: 達成 99% 通關率

### 🏆 里程碑成就
- **通關率突破**: 在 100 次確定性評估中，達成 **99%** 的通關率 (99/100)。
- **平均獎勵**: 達到 **7647** 分 (對應遊戲分數約 6647)。
- **自動化訓練**: 實作 `target_win_rate` 機制，支援目標導向的自動停止訓練。

### 🛠️ 關鍵修復與優化
- **通關訊號修復**: 修正 `environment.py` 中通關時未正確發送 `win` 訊號與 `done` 狀態的 Bug。
- **指標計算修正**: 修正 `train_sb3.py` 中通關率計算方式，從「每步平均」改為「每回合平均」，反映真實勝率。
- **訓練策略**: 採用兩階段訓練法：
    1. **Shock Therapy**: 高學習率/高熵值突破局部最佳解。
    2. **Stabilization**: 低學習率/低熵值穩定收斂，最終達成 99% 勝率。

## [2025-12-04] - 🚀 v5 Refined: 效率與穩定性大躍進

### ⚡ 核心優化 (Efficiency & Stability)
針對 50M 步無法收斂的問題，進行了深度的架構重構與參數優化：

- **Frame Skip (4)**: 
  - 實作每 4 幀執行一次動作，將 500 萬步的訓練量擴展為 2000 萬幀的遊戲體驗。
  - 大幅提升代理的決策視野 (Effective Horizon)，使其能預判更長遠的後果。

- **Death Interrupt (死亡中斷)**:
  - 修復 Frame Skip 迴圈中的重大邏輯陷阱。
  - 新增 `if done: break`，確保代理死亡後立即停止收集數據，杜絕無效的「屍體漫步」數據污染訓練集。

- **輕量化網路架構**:
  - 將 MLP 網路從 `[512, 512]` 縮減為 `[256, 256]`。
  - 減少參數數量以加快收斂速度，並降低在簡單物理環境中過擬合 (Overfitting) 的風險。

- **Linear Learning Rate Schedule**:
  - 實作線性學習率衰減 (`3e-4` → `1e-5`)。
  - 前期大膽探索，後期精細微調，有效提升最終收斂的穩定性。

- **Input Normalization**:
  - 強制啟用 `VecNormalize`，確保所有輸入特徵 (距離、速度) 被標準化，這對 PPO 的性能至關重要。

### 🔧 其他調整
- **移除 Frame Stacking**: 確認物理資訊 (位置、速度) 已足夠，移除冗餘的堆疊以節省記憶體。
- **移除 Speed Reward**: 避免代理為了貪圖速度獎勵而採取自殺式衝刺，回歸純粹的生存與通過獎勵。
