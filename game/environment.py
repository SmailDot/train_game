...
class GameEnv:
    ...
    def step(self, action: int):
        ...
        if 0 < self.ob_x < 2.0 and abs(self.y - ( (self.gap_top + self.gap_bottom)/2 )) < 30:
            reward += 10.0
        ...
        if self.y < 0 or self.y > self.ScreenHeight:
            ...
            # collision with obstacle when x < 10 (approx at player) and y outside gap
    ...
    def render(self):
        ...
        print(f"t={self.t} y={self.y:.1f} vy={self.vy:.1f} ob_x={self.ob_x:.1f} gap=[{self.gap_top:.1f},{self.gap_bottom:.1f}]")
